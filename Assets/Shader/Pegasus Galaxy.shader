...
Shader "ShaderMan/Pegasus Galaxy"
	{
...
{
  fixed2 p  = floor(x);
  fixed2 f  = smoothstep(0.0, 1.0, frac(x));
...
{
  fixed3 p  = floor(x);
  fixed3 f  = smoothstep(0.0, 1.0, frac(x));
...
fixed3x3 m = fixed3x3( 0.00,  1.60,  1.20, -1.60,  0.72, -0.96, -1.20, -0.96,  1.28 );

// Fractional Brownian motion
float fbmslow( fixed3 p )
{
...
fixed4 frag(v2f i) : SV_Target{
{
	...
	fixed2 xy = -1.0 + 2.0*i.vertex.xy / _ScreenParams.xy;
	...
	fixed3 campos = fixed3(500.0, 850., -.0-cos((time-1.4)/2.)*2000.); // moving
	fixed3 camtar = fixed3(0., 0., 0.);
	
	float roll = 0.34;
	fixed3 cw = normalize(camtar-campos);
	fixed3 cp = fixed3(sin(roll), cos(roll),0.0);
	fixed3 cu = normalize(cross(cw,cp));
	fixed3 cv = normalize(cross(cu,cw));
	fixed3 rd = normalize( xy.x*cu + xy.y*cv + 1.6*cw );

	fixed3 light   = normalize( fixed3(  0., 0.,  0. )-campos );
	float sundot = clamp(dot(light,rd),0.0,1.0);

	// render sky

    // galaxy center glow
    fixed3 col = glow*1.2*min(fixed3(1.0, 1.0, 1.0), fixed3(2.0,1.0,0.5)*pow( sundot, 100.0 ));
    // moon haze
    col += 0.3*fixed3(0.8,0.9,1.2)*pow( sundot, 8.0 );

	// stars
	fixed3 stars = 85.5*fixed3(pow(fbmslow(rd.xyz*312.0), 7.0),pow(fbmslow(rd.xyz*312.0), 7.0),pow(fbmslow(rd.xyz*312.0), 7.0))*fixed3(pow(fbmslow(rd.zxy*440.3), 8.0),pow(fbmslow(rd.zxy*440.3), 8.0),pow(fbmslow(rd.zxy*440.3), 8.0));
	
	// moving background fog
    fixed3 cpos = 1500.*rd + fixed3(831.0-time*30., 321.0, 1000.0);
    col += fixed3(0.4, 0.5, 1.0) * ((fbmslow( cpos*0.0035 ) - .5));

	cpos += fixed3(831.0-time*33., 321.0, 999.);
    col += fixed3(0.6, 0.3, 0.6) * 10.0*pow((fbmslow( cpos*0.0045 )), 10.0);

	cpos += fixed3(3831.0-time*39., 221.0, 999.0);
    col += 0.03*fixed3(0.6, 0.0, 0.0) * 10.0*pow((fbmslow( cpos*0.0145 )), 2.0);

	// stars
	cpos = 1500.*rd + fixed3(831.0, 321.0, 999.);
	col += stars*fbm(cpos*0.0021);
	
	
	// Clouds
    fixed2 shift = fixed2( time*100.0, time*180.0 );
    fixed4 sum = fixed4(0,0,0,0); 
    float c = campos.y / rd.y; // cloud height
    fixed3 cpos2 = campos - c*rd;
    ...
    if (radius<1.8)
    {
  	  for (int q=10; q>-10; q--) // layers
      {
        ...
        fixed3 cpos = campos + c*rd;

   		float see = dot(normalize(cpos), normalize(campos));
		fixed3 lightUnvis = fixed3(.0,.0,.0 );
		fixed3 lightVis   = fixed3(1.3,1.2,1.2 );
		fixed3 shine = lerp(lightVis, lightUnvis, smoothstep(0.0, 1.0, see));
		...
		float rot = 3.00*(radius)-time;
      	cpos.xz = mul(cpos.xz,fixed2x2(cos(rot), -sin(rot), sin(rot), cos(rot)));
 	
		cpos += fixed3(831.0+shift.x, 321.0+float(q)*lerp(250.0, 50.0, radius)-shift.x*0.2, 1330.0+shift.y); // cloud position
		cpos *= lerp(0.0025, 0.0028, radius); // zoom
	  	...
	  	fixed3 dustcolor = lerp(fixed3( 2.0, 1.3, 1.0 ), fixed3( 0.1,0.2,0.3 ), pow(radius, .5));
      	fixed3 localcolor = lerp(dustcolor, shine, alpha); // density color white->gray
		  
		float gstar = 2.*pow(noise( cpos*21.40 ), 22.0);
		float gstar2= 3.*pow(noise( cpos*26.55 ), 34.0);
		float gholes= 1.*pow(noise( cpos*11.55 ), 14.0);
		localcolor += fixed3(1.0, 0.6, 0.3)*gstar;
		localcolor += fixed3(1.0, 1.0, 0.7)*gstar2;
		localcolor -= gholes;
        ...
        sum += fixed4(localcolor*alpha, alpha); // sum up weightened color
	  }
  	  ...
  	  for (int q=0; q<20; q++) // 120 layers
      {
		...
		cpos += fixed3(831.0+shift.x, 321.0+float(q)*lerp(250.0, 50.0, radius)-shift.x*0.2, 1330.0+shift.y); // cloud position
      	...
      	fixed3 localcolor = fixed3(0.0, 0.0, 0.0); // density color white->gray
    ...
    sum.rgb -= 0.2*fixed3(0.8, 0.75, 0.7) * pow(sundot,10.0)*alpha;
    sum.rgb += min(glow, 10.0)*0.2*fixed3(1.2, 1.2, 1.2) * pow(sundot,5.0)*(1.0-alpha);

   	col = lerp( col, sum.rgb , sum.w);//*pow(sundot,10.0) );

    // haze
	col = fade*lerp(col, fixed3(0.3,0.5,.9), 29.0*(pow( sundot, 50.0 )-pow( sundot, 60.0 ))/(2.+9.*abs(rd.y)));
    ...
    if (time<2.5)
    {
        ...
        // flicker
		col = fixed3(c, c, c) * (1.-0.5*pow(noise(fixed2(time*99., 0.)), 9.));
    }
    else
    {
        // bam
        float c = clamp(1.-(time-2.5)*6., 0., 1. );
        col = lerp(col, fixed3(1.,1.,1.),c);
    }
#endif
    
    // Vignetting
	fixed2 xy2 = i.vertex.xy / _ScreenParams.xy;
	col *= fixed3(.5, .5, .5) + 0.25*pow(100.0*xy2.x*xy2.y*(1.0-xy2.x)*(1.0-xy2.y), .5 );	
