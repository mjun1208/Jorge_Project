...
#define FUDGE_AMOUNT 0.06
 ...
 struct VertexInput {
    ...
    fuckCoord.x += (sin(screenCoord.x*pow(_Time.w,8.7))*fuckAmt)/64.;
    fuckCoord.y += (cos(screenCoord.y*pow(_Time.w,8.3))*fuckAmt)/56.;
    ...
    if(co.g > 0.005 && ((co.g > co.r + FUDGE_AMOUNT) && (co.g > co.b + FUDGE_AMOUNT))){
        ...
        noise.r = pow(noise.r, 615.);
        noise.g = pow(noise.g, 615.);
        noise.b = pow(noise.b, 615.);
