...
fixed2x2 r2(in float a){ float c = cos(a), s = sin(a); return fixed2x2(c, -s, s, c); }

// fixed2 to float hash.
float hash21(fixed2 p){
   
    float n = dot(p, fixed2(7.163, 157.247)); 
    return frac(sin(n)*43758.5453);
}

// fixed3 to float hash.
float hash31(fixed3 p){
   
    float n = dot(p, fixed3(13.163, 157.247, 7.951)); 
    return frac(sin(n)*43758.5453); 
}
...
fixed3 tex3D( sampler2D tex, in fixed3 p, in fixed3 n ){
  
    n = max((abs(n) - 0.2)*7., 0.001); // n = max(abs(n), 0.001), etc.
    n /= (n.x + n.y + n.z );  
    
	fixed3 tx = (tex2D(tex, p.yz)*n.x + tex2D(tex, p.zx)*n.y + tex2D(tex, p.xy)*n.z).xyz;
    
    return tx*tx;
}

// More concise, self contained version of IQ's original 3D noise function.
float noise3D(in fixed3 p){
    
    // Just some random figures, analogous to stride. You can change this, if you want.
	static const fixed3 s = fixed3(113, 157, 1);
	
	fixed3 ip = floor(p); // Unique unit cell ID.
    
    // Setting up the stride fixedtor for randomization and interpolation, kind of. 
    // All kinds of shortcuts are taken here. Refer to IQ's original formula.
    fixed4 h = fixed4(0., s.yz, s.y + s.z) + dot(ip, s);
...
float fbm(in fixed3 p){
    
    // Four layers of 3D noise.
    //p /= 1.5;
    //p -= fixed3(0, 0, _Time.y*1.);
...
fixed2 path(in float z){ 

    //return fixed2(0);
    
    //return fixed2(sin(z * 0.15)*2.4, cos(z * 0.075)*.15); 
    
    return fixed2(sin(z * 0.15)*2.4, 0);
}
...
// Helper fixedtor. If you're doing anything that involves regular triangles or hexagons, the
// 30-60-90 triangle will be involved in some way, which has sides of 1, sqrt(3) and 2.
static const fixed2 s = fixed2(.866025, 1);//const fixed2 s = fixed2(1, 1.7320508); //


// The 2D hexagonal isosuface function: If you were to render a horizontal line and one that
// slopes at 60 degrees, mirror, then combine them, you'd arrive at the following.
float hex(in fixed2 p){
...
float hexPylon(fixed2 p2, float pz, float r, float ht){

    fixed3 p = fixed3(p2.x, pz, p2.y);
    
    // Note the "*1.5" You need to take the minimum of
    // long-sided rectangles, not squares. Squares will give
    // you a dodecahedron.
    fixed3 b = fixed3(r*1.5, ht, r); 
    
    //p.xz = abs(p.xz);
    //p.xz = fixed2(p.x*.866025 + p.z*.5, p.z);
...
float hexPylon(fixed2 p2, float pz, float r, float ht){
    ...
    p.xz = fixed2(max(p2.x, p2.y), max(p.z, p.x));
    // The ".015" is a subtle rounding factor. Zero gives sharp edges,
    // and larger numbers give a more rounded look.
  	return length(max(abs(p) - b + .015, 0.)) - .015;
    #else
    ...
    p.xy = abs(fixed2(length(p.xz), p.y)) - b.xy + .015;
...
float objDist(fixed2 p, float pH, float r, float ht, inout float id, float dir){
    
    // Neon light height: Four levels, plus the height is divided by two.
    static const float s = 1./16.; //1./4./2.*.5; 
...
float hexHeight(fixed2 p){
    
    // Random height.
    //return hash21(p + 57.)*.75;
    
    // Any kind of cheap flowing height field will do. 
    return dot(sin(p*2. - cos(p.yx*1.4)), fixed2(.25,.25)) + .5;
    
    
    // Two layers. Not used, because we're trying to keep costs down.
    //float n1 = dot(sin(p*2. - cos(p.yx*1.4)), fixed2(.25)) + .5;
    //float n2 = dot(sin(p.yx*8. - cos(p*6.)), fixed2(.25)) + .5;
    //return n1*.85 + n2*.15;
}
...
fixed4 getHex(fixed2 p, float pH){
   
    // The hexagon centers: Two sets of repeat hexagons are required to fill in the space, and
    // the two sets are stored in a "fixed4" in order to group some calculations together. The hexagon
    // center we'll eventually use will depend upon which is closest to the current point. Since 
    // the central hexagon point is unique, it doubles as the unique hexagon ID.
    fixed4 hC = floor(fixed4(p, p - fixed2(0, .5))/s.xyxy) + fixed4(0, 0, 0, .5);
    fixed4 hC2 = floor(fixed4(p - fixed2(.5, .25), p - fixed2(.5, .75))/s.xyxy) + fixed4(.5, .25, .5, .75);
    
    // Centering the coordinates with the hexagon centers above.
    fixed4 h = fixed4(p - (hC.xy + .5)*s, p - (hC.zw + .5)*s);
    fixed4 h2 = fixed4(p - (hC2.xy + .5)*s, p - (hC2.zw + .5)*s);
    
    // Hexagon height.
    fixed4 ht = fixed4(hexHeight(hC.xy), hexHeight(hC.zw), hexHeight(hC2.xy), hexHeight(hC2.zw));
   ...
   static const float r = .25; // .21 to .3. 
    fixed4 obj = fixed4(objDist(h.xy, pH, r, ht.x, litID.x, 1.), objDist(h.zw, pH, r, ht.y, litID.y, -1.), 
    ...
    //
    h = obj.x<obj.y ? fixed4(h.xy, hC.xy) : fixed4(h.zw, hC.zw);
    h2 = obj.z<obj.w ? fixed4(h2.xy, hC2.xy) : fixed4(h2.zw, hC2.zw);
    
    fixed2 oH = obj.x<obj.y ? fixed2(obj.x, litID.x) : fixed2(obj.y, litID.y);
    fixed2 oH2 = obj.z<obj.w ? fixed2(obj.z, litID.z) : fixed2(obj.w, litID.w);
    
    //return oH<oH2 ? fixed4(h.xy, hC.xy) : fixed4(h2.xy, hC2.xy);
    return oH.x<oH2.x ? fixed4(oH,  h.zw) : fixed4(oH2, h2.zw);
...
float heightMap(in fixed3 p){
    ...
    static const float sc = 1.;
    fixed4 h = getHex(p.xz*sc, -p.y*sc);
...
float trace(fixed3 ro, fixed3 rd){
...
/*
void getGlow(fixed3 ro, fixed3 rd, float t){
...
float traceRef(fixed3 ro, fixed3 rd){
...
float softShadow(fixed3 ro, fixed3 lp, float k){

    // More would be nicer. More is always nicer, but not really affordable.
    static const int maxIterationsShad = 32; 
    
    fixed3 rd = (lp-ro); // Unnormalized direction ray.
...
fixed3 getNormal(in fixed3 p) {
	static const fixed2 e = fixed2(0.0025, 0);
...
fixed3 getNormal(fixed3 p, inout float edge, inout float crv, float ef) { 
	
    // Roughly two pixel edge spread, but increased slightly with larger resolution.
    fixed2 e = fixed2(ef/lerp(450., iResolution.y, .5), 0);
    ...
    e = fixed2(.0025, 0); //iResolution.y - Depending how you want different resolutions to look.
	d1 = map(p + e.xyy), d2 = map(p - e.xyy);
	d3 = map(p + e.yxy), d4 = map(p - e.yxy);
	d5 = map(p + e.yyx), d6 = map(p - e.yyx);
...
float calcAO(in fixed3 p, in fixed3 n)
{
	float sca = 4., occ = 0.0;
    for( int i=1; i<6; i++ ){
...
fixed3 texBump( sampler2D tx, in fixed3 p, in fixed3 n, float bf){
   
    static const fixed2 e = fixed2(0.001, 0);
    
    // Three gradient fixedtors rolled into a matrix, constructed with offset greyscale texture values.    
    fixed3x3 m = fixed3x3( tex3D(tx, p - e.xyy, n), tex3D(tx, p - e.yxy, n), tex3D(tx, p - e.yyx, n));
    
    fixed3 g = mul(fixed3(0.299, 0.587, 0.114),m); // Converting to greyscale.
    g = (g - dot(tex3D(tx,  p , n), fixed3(0.299, 0.587, 0.114)) )/e.x; g -= n*dot(n, g);
...
fixed3 envMap(fixed3 p){
    ...
    p = fixed3(c, c*c, c*c*c); // Bluish tinge.
    
    return lerp(p, p.zyx, n3D2*.25 + .75); // lerping in a bit of purple.

}


fixed3 getObjectColor(fixed3 p, fixed3 n){
    ...
    fixed3 col = tex3D(iChannel0, txP*sz0, n );
    col = smoothstep(-.0, .5, col);//*fixed3(.5, .8, 1.5);
    col = lerp(col, fixed3(1,1,1)*dot(col, fixed3(.299, .587, .114)), .5);
    ...
    //if(svLitID==1. && rnd<=.0) col *= fixed3(1, .85, .75)*4.;
    ...
    if(rnd>0. && svLitID==1.) {
    	...
    	ht = floor(ht*4.99)/4./2. + .02;
        static const float s = 1./4./2.*.5; // Four levels, plus the height is divided by two.
     
        oGlow = lerp(1., 0., clamp((abs(p.y - (ht - s)))/s*3.*1., 0., 1.));
        oGlow = smoothstep(0., 1., oGlow*1.);
    }
    
    // lerp the object glow in with a small potion of the volumetric glow.
    glow = lerp(glow, fixed3(oGlow,oGlow,oGlow), .75);
    ...
    //glow = pow(fixed3(1, 1.05, 1.1)*glow.x, fixed3(6, 3, 1));
    glow = pow(fixed3(1.5, 1, 1)*glow, fixed3(1, 3, 6)); // Mild firey orange.
    glow = lerp(glow, glow.xzy, dot(sin(p*4. - cos(p.yzx*4.)), fixed3(.166,.166,.166)) + .5); // lerping in some pink.
    glow = lerp(glow, glow.zyx, dot(cos(p*2. - sin(p.yzx*2.)), fixed3(.166,.166,.166)) + .5); // Blue tones.
    //glow = lerp(glow.zyx, glow, smoothstep(-.1, .1, dot(sin(p + cos(p.yzx)), fixed3(.166))));
...
// standard stuff.
fixed3 doColor(in fixed3 sp, in fixed3 rd, in fixed3 sn, in fixed3 lp, in float t){
    
    fixed3 sceneCol = fixed3(0,0,0);
    ...
    if(t<FAR){
        ...
        fixed3 ld = lp - sp; // Light direction fixedtor.
        float lDist = max(length(ld), .001); // Light to surface distance.
        ld /= lDist; // Normalizing the light fixedtor.
        ...
        fixed3 objCol = getObjectColor(sp, sn);


        // Combining the above terms to produce the final scene color.
        sceneCol = objCol*(diff + fixed3(1, .6, .3)*spec*4. + .5*ao + fixed3(.3, .5, 1)*fres*fres*2.);

        // Fake environment mapping.
        sceneCol += pow(sceneCol, fixed3(1.,1.,1.))*envMap(reflect(rd, sn))*4.;
 ...
 struct v2f {
	...
	// Camera Setup.
	//fixed3 lookAt = fixed3(0., 0.25, _Time.y*2.);  // "Look At" position.
	//fixed3 camPos = lookAt + fixed3(2., 1.5, -1.5); // Camera position, doubling as the ray origin.
	
	fixed3 lk = fixed3(0, 1.25, _Time.y*2.);  // "Look At" position.
	fixed3 ro = lk + fixed3(0, .175, -.25); // Camera position, doubling as the ray origin.

   
    // Light position. Set in the vicinity the ray origin.
    fixed3 lp = ro + fixed3(0, 1, 4); //4
    
	// Using the Z-value to perturb the XY-plane.
	// Sending the camera, "look at," and two light fixedtors down the tunnel. The "path" function is 
    ...
    fixed3 forward = normalize(lk-ro);
    fixed3 right = normalize(fixed3(forward.z, 0., -forward.x )); 
    fixed3 up = cross(forward, right);
    ...
    fixed3 sceneColor, passColor, sn, sSn;
    ...
    sceneColor = passColor;//*(1. - edge*.8);//lerp(passColor, fixed3(0), fog); //
...
/*    
    ...
    rd = normalize(i.worldSpacePosition);
    ...
    passColor = doColor(ro, rd, sn, lp, t);
    sceneColor = sceneColor*.5 + passColor*(1. - edge2*.8);//lerp(passColor, fixed3(0), fog);
